...
class ADB():
    ...
    def run_cmd(self,cmd,timeout=60):
        ...
        if self.__adb_path is None or os.path.isfile(self.__adb_path) is False:
        ...
        try:
            ...
            my_timer = Timer(timeout, kill, [adb_proc])
            my_timer.start()
            ...
            logger.exception("adb run command exception %s." % e)
        finally:
            if my_timer:
                my_timer.cancel()
    ...
    def get_version(self):
        """
        Description:
            get the adb tool version
        Returns: (str) ADB tool version.
        """
        self.run_cmd("version")
        try:
            version_index = self.__output.split().index("version")
            ret = self.__output.split()[version_index + 1]
        except Exception:
            ret = None
    ...
    def get_remote_file(self,remote,local):
        ...
        """
        self.__clean__()
        self.run_cmd(['pull',remote , local], timeout=1200)
